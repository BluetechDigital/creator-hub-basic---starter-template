...
const LatestVideos = async ({ excludeVideoId }: ILatestVideos) => {
	...
	return (
		<section className={styles.latestVideos}>
			<span className={styles.latestVideosEyebrow}>{dict.videos.moreToWatch}</span>
			<h2 className={styles.latestVideosHeading}>{dict.videos.defaultHeading}</h2>
			<div className={styles.latestVideosGrid}>
				{videos.map((video) => (
					<VideoCard key={video.videoId} video={video} dict={dict.videos} />
				))}
			</div>
		</section>
	);
